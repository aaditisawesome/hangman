...
class Hangman:
    ...
    def new_game(self):
        ...
        self.button.config(text="Submit", command= self.guess_define,width=10)
        self.button.pack()
        ...
        self.stopwatch_state = True
        self.root.update()
        timer_txt = self.canvas.create_text(275, 10, text=self.format())
        def update_timer():
            self.canvas.itemconfig(timer_txt, text = self.format())
